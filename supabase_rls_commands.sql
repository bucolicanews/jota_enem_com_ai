...
-- =================================================================================
-- RLS para o novo bucket 'model_avatars' (storage.objects)
-- =================================================================================

-- Habilitar RLS na tabela storage.objects se ainda não estiver habilitado
ALTER TABLE storage.objects ENABLE ROW LEVEL SECURITY;

-- Remover políticas existentes para o bucket 'model_avatars' se houver
DROP POLICY IF EXISTS "Allow authenticated users to view model_avatars" ON storage.objects;
DROP POLICY IF EXISTS "Allow anon users to view model_avatars" ON storage.objects;
DROP POLICY IF EXISTS "Allow Admins/Devs to manage model_avatars" ON storage.objects;

-- Policy 1: Permitir que usuários autenticados visualizem avatares de modelos
CREATE POLICY "Allow authenticated users to view model_avatars"
ON storage.objects
FOR SELECT TO authenticated
USING (bucket_id = 'model_avatars');

-- Policy 2: Permitir que usuários anônimos visualizem avatares de modelos (se o bucket for público)
CREATE POLICY "Allow anon users to view model_avatars"
ON storage.objects
FOR SELECT TO anon
USING (bucket_id = 'model_avatars');

-- Policy 3: Permitir que Admins/Devs gerenciem (inserir, atualizar, deletar) avatares de modelos
CREATE POLICY "Allow Admins/Devs to manage model_avatars"
ON storage.objects
FOR ALL TO authenticated
USING (
  bucket_id = 'model_avatars' AND
  EXISTS (
    SELECT 1 FROM public.cliente
    WHERE id = auth.uid()
    AND permissao_id IN (
